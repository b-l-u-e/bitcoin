...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def test_wallet_reorg_during_background_sync(self, dump_output):
        ...
        n0.invalidateblock(n0.getblockhash(fork_point + 1))
        self.restart_node(0, extra_args=self.extra_args[0])
        n0 = self.nodes[0]
        n0.setmocktime(n3.getblockheader(n3.getbestblockhash())['time'])
        self.connect_nodes(n3.index, n0.index)
        self.generate(n0, nblocks=(orig_height - fork_point) + 2, sync_fun=self.no_op)
        assert int(n0.getblockchaininfo()['chainwork'], 16) > orig_chainwork
